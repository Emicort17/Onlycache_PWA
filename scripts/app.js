...
function fetchCocktail() {
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            if (!cocktail) {
                // HTML corregido (antes había espacios dentro de las etiquetas, lo que rompe el parseo)
                resultDiv.innerHTML = `<p>No se encontró el cóctel: <strong>${cocktailName}</strong></p>`;
                return;
            }
            // Debug: mostrar los datos recibidos en la consola para verificar que strDrink existe
            console.log('Cocktail data:', cocktail);
            // Renderiza el resultado, incluyendo el fallback JSON si es el que se recibió
            resultDiv.innerHTML = `<h2>${cocktail.strDrink}</h2>
